...
function displayNextSentence() {
  if (currentSentenceIndex < sentencesToRecord.length) {
    ...
    sentenceDisplay.textContent = "녹음 문장이 종료되었습니다.";
...
// Add an event listener to the "저장" (Save) button
...
saveBtn.addEventListener('click', () => {
  ...
  // Create a JSON object for user info
  const userInfo = {
    이름: name,
    나이: age,
    성별: gender,
  ...
  const jsonUserInfo = JSON.stringify(userInfo, null, 2);

  // Create a Blob from the JSON string
  const blob = new Blob([jsonUserInfo], { type: 'application/json' });
